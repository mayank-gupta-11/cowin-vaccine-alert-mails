...
def update_rest_api_policy(block_this_ip):
    ...
    #key = 'blocked-ip/blocked-ip-add.txt'
    # read the blocked ip add
    #obj = s3.Object(bucket, key)
    #curr_blocked_ip_add = obj.get()['Body'].read().decode('utf-8')
    ...
    #below one is working which is commented
    #res_policy = {'Version': '2012-10-17', 'Statement': [{'Effect': 'Allow', 'Principal': '*', 'Action': 'execute-api:Invoke', 'Resource': 'arn:aws:execute-api:ap-south-1:328864072647:pt5u9bxa5g/*/POST/'}, {'Effect': 'Deny', 'Principal': '*', 'Action': 'execute-api:Invoke', 'Resource': 'arn:aws:execute-api:ap-south-1:328864072647:pt5u9bxa5g/*/POST/', 'Condition': {'IpAddress': {'aws:SourceIp': ['148.64.12.44/24', '198.51.100.0/24', '198.43.122.0/24']}}}]}
...
def validate_source_ip(ip_add):
    ...
    #bucket = 'cowin-details'
    #key = 'blocked-ip/visited-ip-add.txt'
    # read the ip add
    #obj = s3.Object(bucket, key)
    #curr_ip_count = obj.get()['Body'].read().decode('utf-8')
...
def save_area_pin_details(area_pin):
    ...
    # read the existing area pins
    obj = s3.Object(bucket, key)
    current_pins = eval(obj.get()['Body'].read().decode('utf-8'))
    ...
    # save the new pin with existing pins
                ...
                for pin in area_pin:
                    ...
                    if pin_status == 'valid':
                        ...
                        if flag == 1:
                            ...
                            subscribe_the_user(email,topic_arn)
                            description = 'Your Registration is Success! Please Confirm the Subsciption mail, sent to your Email Id.'
                            print(flag,description)
