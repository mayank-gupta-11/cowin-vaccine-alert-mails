...
table = dynamodb.Table('ip_add_details')

##....................... start storing ip add details in dynamodb .......................##

def update_item(ip_add,count):
    ...
    response = table.update_item(
        ...
        UpdateExpression="set info.count_of_visit=:c",
...
def put_item(ip_add,current_count):
    current_count = str(current_count)
    response = table.put_item(
       Item={
            'ip_add': ip_add,
            'info': {
                'count_of_visit': current_count
            }
        }
    )
    print(response)
...
def get_item(ip_add):
    ...
    try:
        response = table.get_item(Key={'ip_add': ip_add})
        data = response['Item']
        
        current_count = data.get('info').get('count_of_visit')
        print(current_count)
    ...
    print(found,current_count)
    return found,current_count

##....................... blocked ip add details .......................##

def get_blocked_ip_add_details():
    
    blocked_ip_list = []
    
    try:
        response = table.get_item(Key={'ip_add': 'blocked_ip_list'})
        data = response['Item']
        
        blocked_ip_list = data.get('info').get('blocked_ip_list')
        print(blocked_ip_list)
    ...
    return eval(blocked_ip_list)

def update_blocked_ip_add_details(list_of_ip):
    
    list_of_ip = str(list_of_ip)
    
    response = table.update_item(
        Key={
            'ip_add': 'blocked_ip_list'
        },
        UpdateExpression="set info.blocked_ip_list=:c",
        ExpressionAttributeValues={
            ':c': list_of_ip
        },
        ReturnValues="UPDATED_NEW"
...
##....................... end storing ip add details in dynamodb .......................##
...
def update_rest_api_policy(block_this_ip):
    ...
    ## now using dynamodb instead of env variable
    
    blocked_ip_list = get_blocked_ip_add_details()
    print(blocked_ip_list,type(blocked_ip_list))
    curr_blocked_ip_add = blocked_ip_list
    ...
    # now using dynamodb instead of env variables
    update_blocked_ip_add_details(blocked_ip_add)
...
def validate_source_ip(ip_add):
    ...
    found,current_count = get_item(ip_add)
    if found == 'yes':
        current_count = int(current_count) + 1
        
        if current_count > 15:
            flag = 1
            update_rest_api_policy(ip_add)
            deploy_the_api_wd_updated_policy()
        else:
            current_count = str(current_count)
            update_item(ip_add,current_count)
    else:
        put_item(ip_add,current_count)
